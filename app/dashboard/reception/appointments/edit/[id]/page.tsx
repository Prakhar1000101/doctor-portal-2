...
export default function EditAppointmentPage({ params }: { params: { id: string } }) {
  ...
  const fetchAvailableTimeSlots = async (date: Date, appointmentId: string) => {
    try {
      ...
      // If the current time is not in the available slots (date changed), reset the time
  ...
  const handleSave = async () => {
    try {
      ...
      // Validate form
      if (!formData.date || !formData.time || !formData.reason || !formData.patientName) {
      ...
      router.push(`/dashboard/reception/appointments/${params.id}`);
    } catch (error) {
  ...
  return (
    <div className="container mx-auto pt-8">
      <div className="flex items-center mb-8">
        <Link href={`/dashboard/reception/appointments/${params.id}`} className="mr-4">
      ...
      <Card>
        ...
        <CardFooter className="flex justify-between">
          <Button
            variant="outline"
            onClick={() => router.push(`/dashboard/reception/appointments/${params.id}`)}
          >
            Cancel
          </Button>
          <Button 
